...
#wage=MRIO[42*31+2,0:42*31]
#e2=wage/province_employ
#e3=numpy.resize(e2,(31,42))
#load investment ratio
investment=pd.read_excel("employment_province.xlsx",sheet_name="investment")
...
for case in test_id:
    delta=pd.read_csv('delta/capacity_cost_'+case+'.csv',index_col=0).reset_index()
    ...
    for i in  numpy.unique(delta['technology']):
        ...
        In_tmp_cost=delta_tmp.loc[delta_tmp['load_zone']=='West_Inner_Mongolia','capacity_cost'].values+delta_tmp.loc[delta_tmp['load_zone']=='East_Inner_Mongolia','capacity_cost'].values
    ...
    for i in province['Province']:
        ...
        for j in numpy.unique(delta['technology']):
            ...
            for k in [2020,2030,2040]:
                ...
                delta_value_cost=delta_value2.loc[delta_value['period']==k,'capacity_cost']
...
#O&M employment - long-term employment
    ...
    ''' 
    sector=pd.read_excel("2017-MRIO.xlsx",sheet_name="Sector",usecols=[2])
    
    sector_2020=pd.DataFrame(sector['Sector'])
    sector_2030=pd.DataFrame(sector['Sector'])
    sector_2040=pd.DataFrame(sector['Sector'])
    '''
    ...
    for i in technology:
        ...
        multiplier_total.to_excel('input_output_multiplier/dollar_multiplier_capacity_'+case+'.xlsx',index=False)
        ...
        '''
        sector_2020_tmp=numpy.zeros([42,1])
        sector_2030_tmp=numpy.zeros([42,1])
        sector_2040_tmp=numpy.zeros([42,1])
        for j in range(42):
            sector_2020_tmp[j,0]=province_multiplier_sector_2020[numpy.arange(j,1302,31),:].sum(axis=0).sum()
            sector_2030_tmp[j,0]=province_multiplier_sector_2030[numpy.arange(j,1302,31),:].sum(axis=0).sum()
            sector_2040_tmp[j,0]=province_multiplier_sector_2040[numpy.arange(j,1302,31),:].sum(axis=0).sum()
        '''
        ...
        '''
        sector_2020_tmp2=pd.DataFrame(sector_2020_tmp*construction_time.loc[0,i],columns=[i])
        sector_2020=pd.concat([sector_2020,sector_2020_tmp2],axis=1)
                               
        sector_2030_tmp2=pd.DataFrame(sector_2030_tmp*construction_time.loc[0,i],columns=[i])
        sector_2030=pd.concat([sector_2030,sector_2030_tmp2],axis=1)
        
        sector_2040_tmp2=pd.DataFrame(sector_2040_tmp*construction_time.loc[0,i],columns=[i])
        sector_2040=pd.concat([sector_2040,sector_2040_tmp2],axis=1)
        '''
#multiplier=multiplier.sort_index(by=multiplier.columns[1:],axis=1)
    ...
    wage_2020.iloc[:,1:]=wage_2020.iloc[:,1:]*construction_time.iloc[0,:]
    wage_2030.iloc[:,1:]=wage_2030.iloc[:,1:]*construction_time.iloc[0,:]
    wage_2040.iloc[:,1:]=wage_2040.iloc[:,1:]*construction_time.iloc[0,:]
    
    multiplier_2020.to_excel('input_output_multiplier/multiplier_capacity_2020_'+case+'.xlsx',index=False)
    multiplier_2030.to_excel('input_output_multiplier/multiplier_capacity_2030_'+case+'.xlsx',index=False)
    multiplier_2040.to_excel('input_output_multiplier/multiplier_capacity_2040_'+case+'.xlsx',index=False)
    
    wage_2020.to_excel('input_output_multiplier/wage_capacity_2020_'+case+'.xlsx',index=False)
    wage_2030.to_excel('input_output_multiplier/wage_capacity_2030_'+case+'.xlsx',index=False)
    wage_2040.to_excel('input_output_multiplier/wage_capacity_2040_'+case+'.xlsx',index=False)   
    
    '''
    sector_2020.to_excel('input_output_multiplier/sector/sector_capacity_2020_'+case+'.xlsx',index=False)
    sector_2030.to_excel('input_output_multiplier/sector/sector_capacity_2030_'+case+'.xlsx',index=False)
    sector_2040.to_excel('input_output_multiplier/sector/sector_capacity_2040_'+case+'.xlsx',index=False)
    '''
